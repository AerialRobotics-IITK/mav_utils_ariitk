...
void obj_cb_(const mav_utils_msgs::BBPoses &msg) {
  ...
  std::cout << "~Obj Found at x = " << obj_loc_.x << " and y =" << obj_loc_.y
            << std::endl;
  // std::cout << "~Obj Found at x = " << obj_pose_.pose.position.x << " and y
  // ="<<obj_pose_.pose.position.y << std::endl;
}

bool permit_(const geometry_msgs::PoseStamped &msg) {
  if (((msg.pose.position.x < (curr_pose_.pose.position.x) + E) ||
       (msg.pose.position.x > (curr_pose_.pose.position.x) - E)) &&
      ((msg.pose.position.y < (curr_pose_.pose.position.y) + E) ||
       (msg.pose.position.y > (curr_pose_.pose.position.y) - E))) {
    return true;
  } else {
    return false;
...
int main(int argc, char **argv) {
  ...
  ros::Publisher setpt_pub_ =
      nh_.advertise<geometry_msgs::PoseStamped>("command", 100);
  ...
  while (ros::ok()) {
    if (!obj_vis_) {
      ...
      //     // {
      //     //     std::cout << "  On the way to mailbox (last pose).. " <<
      //     std::endl;
      //     //     ros::spinOnce();
      //     // }
      // }
      ...
      // setpt_.pose.position.x=curr_pose_.pose.position.x +
      // obj_pose_.pose.position.x; setpt_.pose.position.y =
      // curr_pose_.pose.position.y + obj_pose_.pose.position.y;
